...
type
  ...
  public
    // ************************* JGB JACOB BOEREMA ***********************************************************************************
    // Collation testing!
    // 12-3-2008: bewaart de naam van de huidige collate sortering
    FCollationScheme: string; // default = empty
    // *******************************************************************************************************************************
...
var S1, S2: string;
    ...
    var
    loValue,loGrid: string;
    begin
      loValue := LowerCase(s1);
      loGrid  := LowerCase(s2);
      if loGrid < loValue then
        Result := -1
      else if loGrid > loValue then
        Result := 1
      else // lowercase = equal; compare original case
      begin
        if s2 > s1 then
    //      Result := -1
          Result := 1
...
begin
  ...
  if lenA > 0 then
    S1 := copy(A,1,lenA);
  if lenB > 0 then
    S2 := copy(B,1,lenB); // error happens here!

  //S1 := 'Jacob'; S2 := 'jacob';  ===> i = 0! Dus AnsiCompareText is NIET wat we willen!
//  i := AnsiCompareText(S1,S2);
  i := MyCompareText(S2,S1);

{ 12-3-2008: bij verwijderen record moet het exacte record gevonden worden dus 0 retourneren
        zal ook gelden voor zoeken of record bestaat! dus hier geheel niet gewenst
  if (i=0) then
    if (lena<lenb) then
      i := -1
    else
...
//var
  {CompareStringFunc: TCompareStringFunc;}{function(
    Locale: LCID;
    dwCmpFlags: DWORD;
    lpString1: PWideChar; cchCount1: Integer;
    lpString2: PWideChar; cchCount2: Integer): Integer stdcall;}

// 16-1-2008 JACOB ZET COLLATION
function TLiteDB.SetCollation: integer;
begin
  if Assigned(FBaseInfo) and Assigned(FBaseInfo.FHandle) then
  begin
    Result := sqlite3_create_collation( FBaseInfo.FHandle,
            PAnsiChar('AnsiNoCase'), SQLITE_UTF8, Self, @SqlCompareFunc);
            // Self hierboven kan elke variabele pointer zijn met data die de gebruiker nodig kan hebben
    if Result = 0 then
      FCollationScheme := 'AnsiNoCase';
  end
  else
    Result := -1;
end;
