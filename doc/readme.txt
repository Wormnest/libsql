...
Please look at the website http://libsql.sourceforge.net/ for documentation.
There is a mailing list to discuss libsql https://lists.sourceforge.net/lists/listinfo/libsql-discussion,
an archive of old messages can be found at http://groups.yahoo.com/group/libsql/

Installation
Make sure the appropiate dll files are in your path or can be found by the application.
You will need sqlite.dll or libmysql.dll on windows, or sqlite.so or libmysqlclient.so on linux.
Those files should be included with this distribution.
You are encouraged to download the newest libraries from the sqlite or mysql website.
libmysql.dll (mysql client interface) and libmysqld.dll (embedded) comes with the mysql server installation.
ODBC support for unix can be provided by (i believe multiple libraries) for example the unixodbc project. Your package manager probably knows it.
There is a document available (see sourceforge libsql, files, documentation) describing how to configure odbc on linux.

Installation as package
Open the libsql.lpk (lazarus) or libsql.dpk(delphi), compile and install.

Installation as components:
Please select 'install new component' from delphi's component menu, and select libsql.pas (or 'vclcomponent.pas', delphi only).
This will install TMyDB and TLiteDB on your component palette.
In this case, make sure you put the dll files somewhere in your system path, for example, in c:\windows or c:\winnt, or any place where delphi can always find them!
...
Embedded MySQL
As of version 0.31 i implemented embedded mysql. There are definitively some issues.
One of those issues is that the embedded mysql library also seems experimental, i encountered some bugs which i cannot help.
Like the mysql_create_database api succeeding if the database does not yet exist, but generating an Access Violation (!) if the database already exists.. You can circumvent that be just doing a query like 'create database somebase', but that is of course only a workaround.
